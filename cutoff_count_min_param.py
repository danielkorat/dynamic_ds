...
class LearnedCountMinEvaluation:
    def __init__(self, test_results, valid_results, test_data, valid_data,
                 lookup_data, save, seed, space_list, n_hashes_list, perfect_order, n_workers, data_name, count_sketch):
        ...
        self.perfect_order = perfect_order
    ...
    @classmethod
    def from_args(cls, a):
        return cls(a.test_results, a.valid_results, a.test_data, a.valid_data, a.lookup_data,
            a.save, a.seed, a.space_list, a.n_hashes_list, a.perfect_order, a.n_workers,
            a.data_name, a.count_sketch)
    ...
    def run(self):
        ...
        assert not (
                    args.perfect_order and args.lookup_data), "use either --perfect or --lookup"
        ...
        if args.lookup_data:
            if args.data_name == 'aol':
                ...
                x_train, y_train = get_data_str_with_ports_list(
                    args.lookup_data)
            log_str += get_stat('lookup data:\n' + '\n'.join(args.lookup_data),
                                x_train, y_train)
        print('data loading time: %.1f sec' % (time.time() - start_t))

        if args.valid_results:
            key = 'valid_output'
            y_valid_ordered, valid_scores = order_y_wkey_list(y_valid,
                                                              args.valid_results,
                                                              key)

        if args.test_results:
            key = 'test_output'
            y_test_ordered, test_scores = order_y_wkey_list(y_test,
                                                            args.test_results,
                                                            key)

        if args.perfect_order:
            assert np.abs(1 - len(y_valid) / len(
                y_test)) < 0.1, "valid and test data should have similar # items"
            ...
            for bcut in b_cutoffs:
                for n_hash in args.n_hashes_list:
                    ...
                    if args.perfect_order:
                        ...
                        bcut, scut = get_great_cut(bcut, y_train, np.floor(
                            max_bcut))  # this has to be y_train
                    else:
                        if bcut < len(y_valid):
                            ...
                            scut = valid_scores[-1]
                    n_cmin_buckets = int(
                        (space * 1e6 - bcut * 4 * cutoff_cost_mul) / (
                                    n_hash * 4))
        ...
        if args.lookup_data:
            min_scut = np.min(
                scut_all)  # no need to store elements that are smaller
        ...
        if args.perfect_order:
            ...
            results = pool.starmap(run_ccm,
                                   zip(repeat(y_sorted), bcut_all, nh_all,
                                       nb_all, repeat(name),
                                       repeat(sketch_type)))
        elif args.lookup_data:
            results = pool.starmap(run_ccm_lookup,
                                   zip(repeat(x_valid), repeat(y_valid), nh_all,
                                       n_cm_all, repeat(lookup_dict), scut_all,
                                       repeat(name), repeat(sketch_type)))
        else:
            results = pool.starmap(run_ccm,
                                   zip(repeat(y_valid_ordered), bcut_all,
            ...
            for j in range(len(valid_results[i])):
                for k in range(len(valid_results[i, j])):
                    log_str += '%s: bcut: %d, # hashes %d, # buckets %d - \tloss %.2f\tspace %.1f\n' % \
                               (name, bcut_all[i, j, k], nh_all[i, j, k],
                                nb_all[i, j, k], valid_results[i, j, k],
                                space_actual[i, j, k])
        ...
        best_scuts = scut_all.reshape(rshape)[
            np.arange(rshape[0]), best_param_idx]
        best_bcuts = bcut_all.reshape(rshape)[
            np.arange(rshape[0]), best_param_idx]
        best_n_buckets = nb_all.reshape(rshape)[
            np.arange(rshape[0]), best_param_idx]
        best_n_hashes = nh_all.reshape(rshape)[
            np.arange(rshape[0]), best_param_idx]
        best_valid_loss = valid_results.reshape(rshape)[
            np.arange(rshape[0]), best_param_idx]
        best_valid_space = space_actual.reshape(rshape)[
            np.arange(rshape[0]), best_param_idx]

        for i in range(len(best_valid_loss)):
            log_str += 'space: %.2f, scut %.3f, bcut %d, n_buckets %d, n_hashes %d - \tloss %.2f\tspace %.1f\n' % \
                       (args.space_list[i], best_scuts[i], best_bcuts[i],
        ...
        if args.perfect_order:
            ...
            results = pool.starmap(run_ccm, zip(repeat(y_sorted), best_bcuts,
                                                best_n_hashes, best_n_buckets,
                                                repeat(name),
                                                repeat(sketch_type)))
        elif args.lookup_data:
            results = pool.starmap(run_ccm_lookup,
                                   zip(repeat(x_test), repeat(y_test),
                                       best_n_hashes,
                                       best_n_buckets - best_bcuts,
                                       repeat(lookup_dict), best_scuts,
                                       repeat(name), repeat(sketch_type)))
        else:
            results = pool.starmap(run_ccm_wscore,
                                   zip(repeat(y_test_ordered),
                                       repeat(test_scores), best_scuts,
                                       best_n_buckets - best_bcuts,
                                       best_n_hashes, repeat(name),
                                       repeat(sketch_type)))
        pool.close()
        pool.join()
...
if __name__ == '__main__':
    ...
    argparser.add_argument("--test_results", type=str, nargs='*', help="testing results of a model (.npz file)",
                           default='')
    argparser.add_argument("--valid_results", type=str, nargs='*', help="validation results of a model (.npz file)",
                           default='')
    ...
    lcm=LearnedCountMinEvaluation.from_args(args)
    lcm.run()
